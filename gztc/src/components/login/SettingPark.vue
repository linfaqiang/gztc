  ...
  <div class="park-select">
    ...
    <div class="park-box">
      <div class="park-box-title">
        当前园区： {{ selectPark.name }}
        <span v-if="!selectPark.name">选择</span>
      </div>
    ...
    <div class="select-park-btn">
      <div :class="btnNextEnable" @click="submitAction">下一步</div>
    </div>
  </div>
</template>
<script>
    ...
    selectParkAction (park, areaIndex, selfIndex) {
      ...
      for (var i = 0; i < this.parkList.length; i++) {
        ...
        for (var j = 0; j < areaList.length; j++) {
          areaList[j].class['park-item-current'] = false
        }
      }
      this.parkList[areaIndex].children[selfIndex].class['park-item-current'] = true
      this.btnNextEnable['sp-submit-btn-disable'] = false
    },
    // 确认操作
    submitAction () {
      if (this.btnNextEnable['sp-submit-btn-disable']) {
        return
      }
      this.$router.push({
        ...
        query: {
          ...
          parkName: this.selectPark.parkName,
