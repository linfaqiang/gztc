<template>
  <div class="meeting-room-list">
    <!--头部-->
    <sub-header title="会议室列表"></sub-header>
    ...
    <van-list class="mrl-list-box" v-model="loading" :finished="finished" @load="onLoad">
      <div class="mrl-item clearfix" v-for="(item) in meetingRoomList" :key="item.id" @click="toMeetingRoomDetail(item.id)">
        ...
        <div class="mrl-item-detail">
          ...
          <div class="mrl-item-name clearfix">
            ...
            <span class="item-property-value"><span v-if="item.cost === 0">免费</span>
...
export default {
  ...
  methods: {
    ...
    /**
     * 会议室列表查询 POST /api/place/cnferenceRoomList
     */
    onLoad () {
      var that = this
      var queryData = {
      ...
      this.$http.post('/api/place/cnferenceRoomList', queryData).then(function (res) {
        that.loading = false
